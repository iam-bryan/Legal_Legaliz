...
import {
  ...
  Card, CardContent, List, ListItem, ListItemText, IconButton, TextField,
...
const DocumentsPage = () => {
  ...
  useEffect(() => {
    if (selectedCaseId) {
      ...
      setDocuments([]); // Clear documents if no case is selected
    }
  }, [selectedCaseId]);
        ...
        <Grid item xs={12} md={5}>
          <Card sx={{ borderRadius: '12px', boxShadow: 3 }}>
            <CardContent>
              ...
              <TextField label="Document Title" required fullWidth sx={{ mb: 2 }} value={docTitle} onChange={e => setDocTitle(e.target.value)} />
              <TextField label="Document Type (Optional)" fullWidth sx={{ mb: 2 }} value={docType} onChange={e => setDocType(e.target.value)} />
              <TextField label="Tags (Comma-separated, Optional)" fullWidth sx={{ mb: 2 }} value={docTags} onChange={e => setDocTags(e.target.value)} />
        ...
        <Grid item xs={12} md={7}>
          <Card sx={{ borderRadius: '12px', boxShadow: 3 }}>
            <CardContent>
              ...
              {loadingDocs && <CircularProgress size={24} />}
              {!loadingDocs && documents.length === 0 && <Typography>No documents found for this case.</Typography>}
                ...
                <List>
                  {documents.map((doc) => (
                    ...
                    >
                      <ListItemText
                        primary={doc.title}
                        secondary={`Name: ${doc.file_name} | Type: ${doc.document_type} | Uploaded: ${new Date(doc.uploaded_at).toLocaleDateString()}`}
                      />
                    </ListItem>
                  ))}
                </List>
