...
const BillingPage = () => {
  ...
  // Base URL for API (needed for direct link to PDF script)
  const API_BASE_URL = process.env.REACT_APP_API_URL || '/api'; // Fallback if .env is missing
    ...
    <Box>
      ...
      <Card sx={{ borderRadius: '12px', boxShadow: 3 }}>
        <CardContent>
          ...
          {!loadingBilling && billingRecords.length === 0 && <Typography sx={{ textAlign: 'center', color: 'text.secondary', py: 2 }}>{selectedCaseId ? 'No records.' : 'Select case.'}</Typography>}
                  ...
                  secondaryAction={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                       ...
                       <IconButton
                         edge="end"
                         size="small"
                         href={`${API_BASE_URL}/billing/invoice_pdf.php?invoice_number=${record.invoice_number}`} // Link to PHP script
